...
Through the usage of one's imagination and possibly the help of visual aids like pictures and maps, each story is weaved together with the efforts of everyone involved, creating a unique experience that will hopefully becomes fond memories in the hearts of those who were present.

## Requirements to Play
In addition to the rules provided by this book, there are a few others items that are critical components of not just Harmonyc Khaos, but all TTRPGs, these include:

- **Character sheet**: Each player character will have a character sheet where they can record their character's data and any other information they believe to be worth writing down related to their character;
- **Dice**: Each player is recommended to have at least one set of polyhedral dice (d4, d6, d8, d10, d12, d20 and d100) which covers the usual ones called by in your average TTRPG ruleset though any form of random number generator (RNG) device is also acceptable, though collecting unique dice that cater to you is itself a fun hobby that the author encourages. In case the GM or adventure module call for a roll that requires a d2, one could make a coin toss or, in case of a d3, a player could, per example, assign three pairs of different sides of a d6 as representing each of the possible outcomes, considering that their probability ends up being the same as if you had a theoretical three-sided die;
- **Deck of Cards**: 
- **Maps**: During combat encounters a map is often used alongside a grid to map out the position of each player character and their NPC allies and adversaries. It also helps to provide a more grounded top-down view of their current predicament, reminiscent of the hobby's origins as a war game. Maps can be either physical or digital, with the latter being often managed through a virtual tabletop (VTT);
- **Miniatures**: Alongside maps, tokens used to represent the aforementioned player characters and NPCs and their location within the map. These can be anything you find that fits your needs, from a chess piece to a lego figure to a custom-made highly detailed figurine;
- **Extras**: Besides the previously mentioned components, some additional ones that are recommended to help the flow of the game and increase the immersion and atmosphere include: 
	- Cards with detailed information on abilities and character options such as skills, spells, boons and banes.
	- Trackers to help keep order of whose turn it is.
...
### Exploration
(Can be freeze time or not, metion baldur gate 3 type of games and how it handles this)
### Encounters
(Basics of combat)
###  Downtime
## Glossary
